...
function setupDelegates() {
   ...
   $container.find('#view-options').click(displayOptionsModal.bind(this));
}

function fetchDocs() {
   var options = encodeOptions( this.options );
   var resource = '/api/model/' + this.model + '/documents?' + $.param(options);
   var apiXhr = $.get(resource, displayDocs.bind(this));
...
function updatePaginationControls(pageDocsCount, totalDocsCount) {
   ...
   $label.html( pageStart + ' - ' + pageEnd + ' of ' + totalDocsCount );
